...
3. 拖拽或粘贴到空白列表区会导入发票；查验单按发票号、付款截图按 OCR 金额尝试绑定到唯一条目。识别失败、没有匹配或存在同金额条目时，需要逐份手动选择。
...
- **材料整理**：付款截图、查验单可在条目详情添加，也可拖到条目卡片或材料区绑定；付款截图会用系统本地 OCR 尝试识别实付金额。金额只对应一个条目时可自动归入；多张截图绑定到同一条目后仍会提示确认合计。
