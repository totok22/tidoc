...
3. 拖拽或粘贴到空白列表区会导入发票；查验单按发票号、付款截图按 OCR 金额尝试绑定到唯一条目。识别失败、没有匹配或存在同金额条目时，需要逐份手动选择。可在设置中关闭付款截图 OCR，关闭后截图直接手动选择条目，不再显示“未识别到付款金额”。
...
打印组件文件缺失或损坏时，软件更新页会显示“需要修复”；即使当前版本已是最新，也可从同一位置重新安装。

## 概念区分

- **报账人**：这张发票属于谁，只需要姓名和审核人。
- **收款信息**：运营组打印 Word 用的姓名、学号、电话、开户行、卡号。
- **识别提醒**：导入后自动产生，如缺明细、明细识别合计与发票总额不一致、抬头未识别；只有抬头分区冲突等会造成材料混入的问题标为严重问题。
- **条目备注**：单张发票的处理记录。
- **条目标签**：用于分类、筛选和批量操作。
- **附件备注**：只描述某个附件文件。
...
- **识别提醒**：缺明细、明细识别合计与发票总额不一致、抬头未识别等自动提示，不阻断材料齐备；抬头分区冲突标为严重问题。
- **材料整理**：付款截图、查验单可在条目详情添加，也可拖到条目卡片或材料区绑定；付款截图默认用系统本地 OCR 尝试识别实付金额，设置中可关闭。OCR 开启时，金额只对应一个条目才自动归入；关闭后不读取截图金额并直接手动绑定。“实付默认等于发票金额”是独立设置：开启时新条目自动填写发票金额，关闭时留空待确认，已有条目不受切换影响。
- **防误操作**：同一条目重复附件会被拦截；发票 PDF / XML / 查验单拖入已有条目时会尽量核对发票号，避免混入其他发票；查验单发票号或付款金额不能唯一确定条目时不会自动绑定，必须逐份手动选择；非官方电子发票 XML 会被拒绝。
- **条目管理**：搜索、统一筛选、排序、多栏列表、Shift 范围选择、批量修改报账人、字段修改追踪、材料齐备状态自动计算。
...
- **打印导出**（可选）：默认按每个条目依次拼接发票、付款截图和查验单，便于逐条审查；也可按材料类型分别导出，并可关闭条目编号 / 页码叠加。报账说明 / 验收单 Word 与全部 PDF 均按抬头强隔离。
