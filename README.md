...
3. 拖拽到空白列表区会批量导入发票；拖拽到条目卡片会绑定付款截图或查验单，并校验是否属于当前发票。
4. 可用抬头、报账人、批次、状态、日期、金额、条目标签、条目备注等条件筛选；勾选条目后再批量处理。
...
- **材料整理**：付款截图、查验单可在条目详情添加，也可拖到条目卡片或材料区绑定；实付金额默认按发票总额，上传付款截图时可快速确认或改成实际金额。
- **防误操作**：同一条目重复附件会被拦截；发票 PDF / XML / 查验单拖入已有条目时会尽量核对发票号，避免混入其他发票；非官方电子发票 XML 会被拒绝。
...
> **注意**：阿里云 OCR 识别组件当前未实现，也不在当前计划内。核心只在查验单归属确认时使用系统原生 OCR 兜底读取发票号；发票本身仍以 XML + PDF 文本解析为主。
