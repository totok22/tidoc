...
- 付款截图：支持一张发票多张截图；实付金额默认等于发票总额，上传图片时用系统本地 OCR 尝试识别付款金额。批量拖入时，识别金额只对应一个候选条目才自动绑定；没有匹配、识别失败或存在同金额条目时逐份手动选择。单张绑定成功且当前实付仍是默认值时可直接填入；多张截图识别成功时提示用户确认合计或手动填写。
...
### 8.7 分类 / 筛选 / 搜索
按抬头、报账人、日期、销售方、金额区间、状态、条目标签、条目备注、关键词过滤；常用视图（全部、待补材料、识别提醒、已修改、齐备）作为筛选条内的快捷条件，不拆成左侧多个“页面”。其中“已修改”仅指实付金额与发票总金额不一致，其他字段的修改仍正常留痕，但不进入该视图。通过卡片勾选、Shift 范围选择和 Cmd/Ctrl+A 选中条目后，再做汇总 / 导出 / 打印 / 删除。
